 ...
 */
 /**Simon 20120806.1  _$basePath have been defined in 'common_head.jsp'  */
var $dp, WdatePicker;
(function() {
	var $ = {
		$langList: [{
	...
	function I(G, _) {
		...
		function E(F, E) {
			...
			F.skin = E.skin;
			var G = [];
			/*by Simon 20120806.1 _$basePath have been defined in 'common_head.jsp' */
			if(_$basePath){
				G.push("<base href='"+_$basePath+"'/>");
			}			
			G.push("<head><script>", "var $d, $dp, $cfg=document.cfg, $pdp = parent.$dp, $dt, $tdt, $sdt, $lastInput, $IE=$pdp.ie, $FF = $pdp.ff,$OPERA=$pdp.opera, $ny, $cMark = false;", "if($cfg.eCont){$dp = {};for(var p in $pdp)$dp[p]=$pdp[p];}else{$dp=$pdp;};for(var p in $cfg){$dp[p]=$cfg[p];}", "document.oncontextmenu=function(){try{$c._fillQS(!$dp.has.d,1);showB($d.qsDivSel);}catch(e){};return false;};", "</script><script src=", A, "lang/", H.name, ".js charset=", H.charset, "></script>");
