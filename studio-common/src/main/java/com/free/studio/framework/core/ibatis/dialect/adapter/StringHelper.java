...
public final class StringHelper {
	...
	public static String replace(String beforePlaceholder, String afterPlaceholder, String placeholder,
		...
		boolean actuallyReplace = (!wholeWords) || (afterPlaceholder.length() == 0)
	...
	public static int firstIndexOfChar(String sqlString, BitSet keys, int startindex) {
		int i = startindex;
		for (int size = sqlString.length(); i < size; i++) {
	...
	public static int firstIndexOfChar(String sqlString, String string, int startindex) {
		...
		int i = 0;
		for (int size = string.length(); i < size; i++) {
	...
	private static String generateAliasRoot(String description) {
		String result = truncate(unqualifyEntityName(description), 10).toLowerCase().replace('/', '_').replace('$',
				'_');
	...
	public static StringBuilder buildBatchFetchRestrictionFragment(String alias, String[] columnNames,
		...
		if (columnNames.length == 1) {
			return new StringBuilder(qualify(alias, columnNames[0])).append(" in (").append("$$BATCH_ID_PLACEHOLDER$$")
					.append(")");
		}
		if (dialect.supportsRowValueConstructorSyntaxInInList()) {
			...
			for (String columnName : columnNames) {
				builder.append(deliminator).append(qualify(alias, columnName));
				if (firstPass) {
					firstPass = false;
					deliminator = ",";
				}
	...
	public static String expandBatchIdPlaceholder(String sql, Serializable[] ids, String alias, String[] keyColumnNames,
			Dialect dialect) {
		if (keyColumnNames.length == 1) {
			return replace(sql, "$$BATCH_ID_PLACEHOLDER$$", repeat("?", ids.length, ","));
		}
		if (dialect.supportsRowValueConstructorSyntaxInInList()) {
			String tuple = "(" + repeat("?", keyColumnNames.length, ",");
			return replace(sql, "$$BATCH_ID_PLACEHOLDER$$", repeat(tuple, ids.length, ","));
		}
		String keyCheck = joinWithQualifier(keyColumnNames, alias, " and ");
		return replace(sql, "$$BATCH_ID_PLACEHOLDER$$", repeat(keyCheck, ids.length, " or "));
	}

	public static String[] toArrayElement(String s) {
		return new String[] { (String) ((s == null) || (s.length() == 0) ? new String[0] : s) };
	}
}
